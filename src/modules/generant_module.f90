module generant_module
  ...
  type gen
     real(DP):: tflame, mpkg, genmass, genheight, gendiam, rhosolid, ntabs, rref, prefn
  end type
  ...
  subroutine set_ntabs(this) ! only called once to get ntabs and sets the value
    ...
    real(DP) :: r,h,mass,m1,rho,vol
    real(DP), parameter :: pi=3.14159265_DP

    r    = this%gendiam/2._DP
    h    = this%genheight
    mass = this%genmass
    rho  = this%rhosolid
    vol  = pi*r**2.0_DP*h    ! tablet volume (cylinder)
